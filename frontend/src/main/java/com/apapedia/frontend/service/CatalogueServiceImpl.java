...
public class CatalogueServiceImpl implements CatalogueService {
    ...
    public List<CategoryResponseDTO> getAllCategory(HttpServletRequest request) {
        ...
        return this.webClient.get().uri(url).header(HttpHeaders.AUTHORIZATION, "Bearer " + getJwtFromCookies(request))
                .retrieve().bodyToFlux(CategoryResponseDTO.class).collectList().block();
    }
    ...
    public List<ReadCatalogueResponseDTO> getAllCatalogue(HttpServletRequest request) {
        ...
        if (token != null) {
            ...
            var listCatalogue = this.webClient.get().uri(url)
                    .header(HttpHeaders.AUTHORIZATION, "Bearer " + getJwtFromCookies(request)).retrieve()
                    .bodyToFlux(ReadCatalogueResponseDTO.class).collectList().block();
    ...
    public ReadCatalogueResponseDTO getCatalogueById(UUID id, HttpServletRequest request) {
        ...
        return this.webClient.get().uri(url).header(HttpHeaders.AUTHORIZATION, "Bearer " + getJwtFromCookies(request))
                .retrieve().bodyToMono(ReadCatalogueResponseDTO.class).block();
    }
    ...
    public List<ReadCatalogueResponseDTO> listCatalogueFiltered(String productName, HttpServletRequest request) {
        ...
        var token = getJwtFromCookies(request);
        try {
            var listCatalogue = this.webClient.get().uri(url).retrieve().bodyToFlux(ReadCatalogueResponseDTO.class)
                    .collectList().block();

            for (var catalogue : listCatalogue) {
                if (token != null && catalogue.getIdSeller().equals(UUID.fromString(getIdFromJwtToken(token)))
                        || token == null) {
                    catalogue.setImageString(Base64.getEncoder().encodeToString(catalogue.getImage()));
                } else {
    ...
    @Override
    public List<ReadCatalogueResponseDTO> getCatalogueListSorted(String sortBy, String order,
            HttpServletRequest request) {
